...
from pipeline_videogen import VideoGenPipeline, OpenSoraPlanConfig, OpenSoraPlanPABConfig
...
def main(args):
    ...
    videogen_pipeline = VideoGenPipeline(config=config,
                                         vae=vae,
    ...
    for prompt in args.text_prompt:
        ...
        end_time=time.time()
        duration = end_time - start_time  # 单次生成所需时间
        durations.append(duration)
        print(f"{prompt}: {duration:.2f} seconds")
    print(durations)
