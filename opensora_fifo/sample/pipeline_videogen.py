...
class OpenSoraPlanConfig:
    def __init__(
        ...
        self.pipeline_cls = VideoGenPipeline
        if self.enable_pab and pab_config is None:
            self.pab_config = OpenSoraPlanPABConfig()
        else:
...
class VideoGenPipeline(VideoSysPipeline):
    r"""
    ...
    def encode_prompt(
        ...
        # 判断文本输入是字符串还是列表，从而确定批次大小（一次处理多少文本输入）
        ...
        max_length = 300
        # 如果没有提供文本嵌入，就使用tokenizer和encoder对文本进行编码
        if prompt_embeds is None:
            prompt = self._text_preprocessing(prompt, clean_caption=clean_caption)
            # 把文本prompt转化为token序列，并生成注意力掩码，注意力掩码用于去除文本中不重要的信息
            ...
            # 通过文本编码器将 token 转换为嵌入（prompt_embeds）
        ...
        # 如果指定了 num_images_per_prompt > 1，即每个 prompt 需要生成多张图片，那么嵌入需要进行重复，确保每个 prompt 生成多个图像
        prompt_embeds = prompt_embeds.repeat(1, num_images_per_prompt, 1)
        #在前面，已经通过 prompt_embeds = prompt_embeds.repeat(1, num_images_per_prompt, 1) 进行了嵌入复制，
        # 将其形状从 (batch_size, seq_len, embed_dim) 扩展为 (batch_size, num_images_per_prompt * seq_len, embed_dim)。
        # 但是，此时 batch_size 还是保持不变，而模型期望的是形状为 (batch_size * num_images_per_prompt, seq_len, embed_dim)。
        # 这样每个prompt会变成多个重复的嵌入
    ...
    # 根据调度器的要求准备一些额外的参数，不同调度器需要的参数不同，比如eta参数只有DDIM需要,eta用于控制噪声的程度
    ...
    def prepare_latents(self, batch_size, num_channels_latents, num_frames, height, width, dtype, device, generator,
                        latents=None):
        # 计算出潜变量的形状，与模型匹配，vae_scale_factor是vae的缩放因子，对图像尺寸进行缩放
        shape = (
            batch_size,
        ...
        if latents is None:
            # 根据前面计算的shape在没有潜变量时，就可以随机生成潜变量
            latents = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
        else:
            latents = latents.to(device)

        # scale the initial noise by the standard deviation required by the scheduler
        # 生成的潜变量（初始噪声）会乘以调度器（scheduler）指定的标准差 init_noise_sigma。这是在扩散模型中，通常在生成图像时会对初始噪声进行缩放，
        # 以便与目标图像保持一致，为什么这里说潜变量是初始噪声，因为在扩散模型推理过程中，最开始就是从随机噪声出发的
