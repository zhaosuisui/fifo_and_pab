...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # 正则化层（根据传入的参数决定使用的归一化层）组归一，空间归一，交叉注意力归一
        ...
        # 根据 USE_PEFT_BACKEND 的值来选择使用标准的线性层 nn.Linear 还是 LoRACompatibleLinear
        else:
        ...
        # 如果是只使用交叉注意力，kv不需要计算，来自于别的源自注意力机制才需要通过线性层得到k和v,q=k=v
    ...
    def set_use_memory_efficient_attention_xformers(
        ...
        Args:
            ...
            attention_op (`Callable`, *optional*):
                ...
                `xformers`.
        """
        # lora custom_diffusion, ..三个注意力处理器是不一样的
        ...
        if use_memory_efficient_attention_xformers:
            ...
            if not is_xformers_available():
                ...
                try:
                    ...
                    raise e
            # 在 Transformer 模型的注意力机制中，通常会计算 Q（Query）、K（Key）、V（Value） 矩阵，来完成自注意力或交互注意力的计算。LoRA 通过插入低秩矩阵，使得权重更新时可以只微调少量参数，而不需要更新整个权重矩阵
            # 把三个处理器load进来，这里的都是写了Xformer,应该有加速作用。基于 xformers 库的标准注意力实现，旨在提供高效的计算性能和内存使用。
            if is_lora:
    ...
    # 返回当前使用的注意力处理器,如果启用了lora,初始化lora处理器
    ...
    def head_to_batch_dim(self, tensor: torch.Tensor, out_dim: int = 3) -> torch.Tensor:
        ...
        return tensor
    # 用于计算注意力机制中的注意力得分。其核心思想是将 query（查询）和 key（键）进行矩阵乘法以生成注意力得分，之后再对其应用 softmax 函数来计算注意力概率。这些概率将用于对 value（值）进行加权求和，完成注意力的计算。
    ...
    # encoder_hidden_states就是经过编码器之后的输出，也就是经过几层注意力之后的结果
...
class BasicTransformerBlock_(nn.Module):
    ...
    def forward(
        ...
        if is_fifo:
            f = timestep.shape[1]
            org_timestep = int(timestep[0, 0, 0].item())  # 提取 (0, 0, 0) 位置的标量值
        else:
            org_timestep = int(timestep.view(-1)[0].item()) # 非 FIFO 模式，假设 timestep 为一维张量

        batch_size = hidden_states.shape[0]
        broadcast_temporal, self.count = if_broadcast_temporal(org_timestep, self.count)
        # 确定使用哪个归一化层
        if self.use_ada_layer_norm:
            norm_hidden_states = self.norm1(hidden_states, timestep)
        # 当前正在使用自适应层归一化（AdaLayerNorm）。这种归一化方法不仅考虑隐藏状态，还根据时间步（timestep）进行动态调整，适用于处理时序数据的模型
        elif self.use_ada_layer_norm_zero:
        ...
        # 标准的层归一化（LayerNorm），那么只会对 hidden_states 进行标准化处理，时间步或其他额外信息不会影响归一化的计算。
        # 如果使用了fifo,一次处理f帧
        # 把broacast_temporal加进去了
        elif self.use_ada_layer_norm_single:
            if is_fifo and broadcast_temporal:
            ...
            elif is_fifo and not broadcast_temporal:
                shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                        self.scale_shift_table[None, None] + timestep.reshape(batch_size, f, 6, -1)
                ).chunk(6, dim=2)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if enable_pab() and broadcast_mlp:
            ff_output = get_mlp_output(
                broadcast_range,
                timestep=int(org_timestep[0].item()),
                block_idx=self.block_idx,
                is_temporal=False,
            )
        else:
